...
This Python-based tool is designed for environmental professionals and researchers to perform quick analysis and visualization of air quality CAAQMS datasets — 
without needing to write code, even rename downloaded file, or sort data, or even work on excel.

Download executable tool from here- https://github.com/atiquebarudgar/My_personal_projects/releases
To download CAAQMS data- visit this website-https://airquality.cpcb.gov.in/ccr/#/caaqm-dashboard-all/caaqm-landing/data
You can check sample CAAQMS data uploaded in the same folder of Readme.txt
Please note every prompt from the tool is case sensitive- type exact what is ask to input.
It will be better if tool file and data file are inside same folder, but not necessary.

## 🚀 Usage

1. **Run the executable (.exe application file)**
   -Double click on CAAQMS_Tool application file.
...
4. **Prompt: "Select Pollutants for Hourly Average Plot"**
    - Above prompt will give list of available pollutants- Enter pollutant names separated by commas if selecting more than one. (case sensitive- upper case)
